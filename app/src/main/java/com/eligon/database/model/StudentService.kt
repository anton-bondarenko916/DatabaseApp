...
typealias StudentsListener = (users: List<Student>) -> Unit

class StudentService(private val databaseHelper: StudentDatabaseHelper) {
    var students = mutableListOf<Student>()

    private val listeners = mutableSetOf<StudentsListener>()


    fun deleteStudent(student: Student) {
        val indexToDelete = students.indexOfFirst { it.id == student.id }
        if (indexToDelete != -1) {
            students.removeAt(indexToDelete)
            databaseHelper.deleteStudent(databaseHelper.writableDatabase, student)
            notifyChanges()
        }
    }
    ...
    companion object {
        ...
        private var numberOfStudents = 0
        ...
        fun generateRandomStudent(): Student {
            ...
            val weight = nextInt(40, 120)
            val height = nextInt(150, 210)
            val age = nextInt(17, 60)
            val photo = IMAGES[numberOfStudents % IMAGES.size]
            numberOfStudents++
            return Student(numberOfStudents, name, weight, height, age, photo)
        }
